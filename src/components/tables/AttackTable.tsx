...
const columns = [
  columnHelper.accessor((row) => row.timestamp, { id: 'timestamp', header: 'Timestamp', cell: (info) => new Date(info.getValue()).toLocaleString() }),
  columnHelper.accessor((row) => row.source_country ?? row.sourceIp ?? row.country, { id: 'source', header: 'Source' }),
  columnHelper.accessor((row) => row.destination_country ?? row.destination, { id: 'destination', header: 'Destination' }),
  columnHelper.accessor((row) => row.attack_type ?? row.attackType, { id: 'attack', header: 'Attack' }),
      ...
      <table className="w-full text-left text-sm text-slate-300">
        ...
        <tbody>
          {table.getRowModel().rows.length ? table.getRowModel().rows.map((row) => (
            ...
            <tr><td className="px-4 py-6 text-slate-500" colSpan={columns.length}>No attacks available</td></tr>
          )}
        </tbody>
      </table>
    </div>
